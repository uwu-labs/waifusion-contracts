...
var ownerAddress, ownerAccount, ownerNFTId, ownerNFTId1, ownerNFTId2, ownerNFTIdOthers;
...
describe("Setup", function () {
  before(async () => {
    ...
    // get main contracts
    WaifuDungeon = await ethers.getContractFactory('WaifuDungeonBSC');
    ...
    wet.approve(waifusion.address, "9999999999999999999999999999999");
  ...
  it("Should let a user buy 1 NFT", async function () {
    ...
    ownerNFTId1 = await waifusion.totalSupply() - 1
    expect(await waifusion.ownerOf(ownerNFTId1)).to.equal(ownerAddress);
  })
...
describe("WaifuDungeon", function () {
  ...
  it("Should commit swap 1 waifus for 0.25 BNB", async function () {
    ...
    await waifuDungeon.commitSwapWaifusWithETH([ownerNFTId], {value: ethers.utils.parseEther("0.25")});
  });
  ...
  it("Should allow modifying the burn perc", async function() {
    await waifuDungeon.setBurnPerc(ethers.utils.parseEther("0.25"));
    expect(await waifuDungeon.burnPerc()).to.equal(ethers.utils.parseEther("0.25"));
  })

  it("Should commit swap 1 waifus for 5490 WET", async function () {
    await waifusion.setApprovalForAll(waifuDungeon.address, true);
    await wet.approve(waifuDungeon.address, "9999999999999999999999999999999");
    expect(await waifusion.isApprovedForAll(ownerAddress, waifuDungeon.address)).to.equal(true)
    await waifuDungeon.commitSwapWaifus([ownerNFTId1]);
    expect(await wet.balanceOf("0x0000000000000000000000000000000000080085")).to.equal(ethers.utils.parseEther("1372.5"))
  });

  it("Should allow rescuing WET", async function() {
    let balanceBefore = await wet.balanceOf(ownerAddress);
    expect(balanceBefore).to.equal("13229395254629629629625")
    await waifuDungeon.rescue(wet.address);
    let balanceAfter = await wet.balanceOf(ownerAddress);
    expect(balanceAfter).to.equal("17346895254629629629625")
  })
  ...
  it("Should commit swap 2 waifus for 0.5 BNB", async function () {
    ...
    await waifusion.setApprovalForAll(waifuDungeon.address, true);
    expect(await waifusion.isApprovedForAll(ownerAddress, waifuDungeon.address)).to.equal(true)
    await waifuDungeon.commitSwapWaifusWithETH([ownerNFTIdOthers[0], ownerNFTIdOthers[1]], {value: ethers.utils.parseEther("0.5")});
  });
